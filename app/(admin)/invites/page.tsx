...
import { NewInvite, NewInviteButton } from "@/components/invites/new-invite";
...
export default async function InvitesPage() {
  ...
  return (
    <NewInvite profiles={profileOptions} defaults={inviteDefaults(invites[0], profiles.map((p) => p.id))}>
      <div className="space-y-4">
        <PageHeader title="Invites" count={invites.length} description="Links that let people create their own account with the profile you choose." actions={<NewInviteButton />} />
        <InvitesTable invites={invites} profiles={profileOptions} links={links} action={<NewInviteButton variant="outline" />} />
      </div>
    </NewInvite>
  );
}
