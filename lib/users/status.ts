...
export function computeUserStatus(isDisabled: boolean, meta: MetaBits | null | undefined, now: Date = new Date()): UserStatus {
  if (meta?.deleteAfter) {
    return { kind: "deletion_scheduled", label: "Deletion scheduled", tone: "red", at: meta.deleteAfter };
  }
  if (isDisabled) {
    if (meta?.disabledByAppAt && meta.disabledReason) {
      return { kind: "disabled_by_app", label: reasonLabel[meta.disabledReason], tone: "red", reason: meta.disabledReason, at: meta.disabledByAppAt };
    }
    return { kind: "disabled", label: "Disabled", tone: "red" };
  }
  if (meta?.expiresAt) {
    const days = daysBetween(meta.expiresAt, now);
    if (days <= 0) return { kind: "expired", label: "Expired", tone: "amber", at: meta.expiresAt };
    if (days <= EXPIRING_SOON_DAYS) return { kind: "expiring", label: "Expires soon", tone: "amber", at: meta.expiresAt };
  }
  return { kind: "enabled", label: "Enabled", tone: "green" };
}
