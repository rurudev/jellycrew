...
- Tables are shadcn's, tuned in `components/ui/table.tsx`: 36 px rows, 12 px muted headers without uppercase, a header that stays put on wide screens, a bordered card surface only when the table stands alone (`variant="plain"` inside a Section). Sortable headers are real links with `aria-sort`; `LinkRow` opens the row's page on a click that lands on nothing interactive, while the name stays a real link, so tooltips and buttons inside rows keep working (no stretched-link overlay).
