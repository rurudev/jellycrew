...
- Result messages still travel as `?ok=` / `?error=` redirects (`redirectWithNotice` in `lib/notice.ts`, `backToUser` for the user page), but they are shown as Sonner toasts by one reader in the root layout that strips the params from the URL with `history.replaceState`, so a reload, bookmark or the back button never replays a message. Success toasts auto-dismiss; error toasts stay until dismissed. The Sonner wrapper takes its theme from the layout (the preference cookie) rather than `next-themes`, which is not installed.
