...
- `Field` is a plain server-compatible component that takes `id`, `label`, `help` and `error` as props and clones them onto its single control (`id`, `aria-describedby`, `aria-invalid`), rather than the context-based compound the plan sketched: no client boundary, and it works identically inside client forms.
- Buttons come in three heights (28 / 32 / 44 px) and never take width classes; `block` stretches them. `LinkButton` renders a `next/link` styled as a button so navigation never nests a button inside a link. `SubmitButton` reads `useFormStatus`, so plain server-action forms get pending state without local code.
- Controls take `width="auto"` when a className sizes them, because `cn()` does not resolve conflicting Tailwind classes and `w-full` would otherwise win over `w-40`.
...
- Every server-action `catch` starts with `unstable_rethrow(err)`: `redirect()` and `notFound()` work by throwing, and six actions were catching their own redirect and surfacing "NEXT_REDIRECT" as an error notice after a successful adopt, copy, schedule-deletion, reset-link or lifecycle run.
