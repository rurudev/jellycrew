...
| 7 | User detail layout | `design/user-detail-layout` | done, merged 2026-09-11 |
| 8 | User actions menu and dialogs | `design/user-actions` | done, merged 2026-09-12 |
...
| 16 | Polish, sweep, document | `design/polish` | done, merged 2026-09-12 |
...
- **Definition of done**: `pnpm lint`, `pnpm typecheck`, `pnpm test:unit` pass; `pnpm test:integration` also passes when the package touches anything under `lib/`; `/code-review` run and findings addressed; no `zinc-*` or `dark:` classes remain in files the package touched (files generated by the shadcn CLI are exempt: their `dark:` utilities follow the theme through the custom variant); one line per design decision added to `DECISIONS.md` under a new "Design" heading; the Status table above updated.
...
**What changes.**
- Table: 36 px rows, hairline separators, hover and selected fills, header cells at `xs` muted 500 with no uppercase, cells that wrap unless they opt out, horizontal scrolling when needed; `SortHead` renders a real link with `aria-sort` and an icon; `LinkRow` (its own client file) opens the row on a click that lands on nothing interactive and outside the selection cell, while the name stays a real link (a stretched-link overlay was rejected: it would block the tooltips and buttons inside rows; a sticky header was rejected: scroll containers and cards clip it).
